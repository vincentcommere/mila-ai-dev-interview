# app/config.py
from pydantic_settings import BaseSettings
from pydantic_settings import SettingsConfigDict
from pydantic import Field
from pydantic import SecretStr
import os


class Settings(BaseSettings):
    # -----------------------------
    #   MANDATORY
    # -----------------------------

    API_KEY: SecretStr = Field(..., description="API key (sécurisée)")

    # -----------------------------
    #   APP SETTINGS
    # -----------------------------
    APP_NAME: str = Field(default="Nvidia Earnings Call Assistant API")
    APP_VERSION: str = Field(default="1.0.0")
    ENV: str = Field(default="development")

    # -----------------------------
    #   CHROMA SETTINGS
    # -----------------------------
    CHROMA_HOST: str = Field(default="chroma")
    CHROMA_PORT: int = Field(default=8000)

    # -----------------------------
    #   MODEL API SETTINGS
    # -----------------------------
    API_URL: str = Field(
        default="https://router.huggingface.co/v1/chat/completions",
        description="URL du provider LLM"
    )
    MODEL: str = Field(
        default="meta-llama/Llama-3.1-8B-Instruct",
        description="Nom du modèle LLM"
    )
    PROVIDER: str = Field(default="fireworks-ai")
    RAG_COLLECTION: str = Field(default="nvidia_earnings_calls")

    # -----------------------------
    #   EMBEDDINGS
    # -----------------------------
    EMBEDDING_MODEL: str = Field(default="BAAI/bge-large-en-v1.5")
    K: int = Field(default=10)
    
    # -----------------------------
    #   PATHS FOR LOCAL DATA
    # -----------------------------
    PROMPTS_DIR: str = Field(default="/prompts")
    PROMPT_FILENAME: str = Field(default="nvidia_prompt.txt")
    RAG_PROMPT_FILENAME: str = Field(default="nvidia_rag_prompt.txt")


    @property
    def PROMPT_FILE(self) -> str:
        return os.path.join(self.PROMPTS_DIR, self.PROMPT_FILENAME)

    @property
    def RAG_PROMPT_FILE(self) -> str:
        return os.path.join(self.PROMPTS_DIR, self.RAG_PROMPT_FILENAME)

    # -----------------------------
    #   LOADING CONFIG
    # -----------------------------
    model_config = SettingsConfigDict(
        env_file=".env",
        env_file_encoding="utf-8",
        case_sensitive=False,
        extra="forbid",   # interdit les inconnues (comportement normal)
    )


settings = Settings()
